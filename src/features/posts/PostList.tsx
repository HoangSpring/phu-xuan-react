    ...
    return (
        <section>
            ...
            <ul style={{ listStyle: 'none', padding: 0 }}>
                {posts.map(post => (
                    <li key={post.id} style={{ border: '1px solid #ddd', borderRadius: 8, padding: '1rem', marginBottom: '1rem' }}>
                        <h3>{post.title}</h3>
                        ...
                        <div>
                            {post.tags.map(tag => (
                                <span key={tag} style={{ marginRight: 6, background: '#f0f0f0', padding: '2px 8px', borderRadius: 4, fontSize: '0.85em' }}>
                                    #{tag}
                                </span>
                            ))}
                        </div>
